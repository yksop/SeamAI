"""
Author: Hanna Norberg
e-mail: hanna.gjelstrup.norberg@gmail.com
"""

import os
import json
import numpy as np
from sklearn.preprocessing import StandardScaler
from sklearn.model_selection import train_test_split

def load_master_json(json_path):
    """Load a JSON file."""
    with open(json_path, 'r') as f:
        return json.load(f)

def calculate_frames_from_seconds(fps, seconds):
    """Calculate the number of frames corresponding to a time horizon in seconds."""
    return int(round(fps * seconds))

def get_video_fps(video_data):
    return video_data.get("fps", 12) # Default to 12 fps if not specified

def pad_sequences(sequence, target_length, pad_value=0.0):
    """Pad a sequence to target length."""
    current_length = len(sequence)

    if current_length == target_length:
        return sequence
    elif current_length < target_length:
        last_frame = sequence[-1]
        padding_count = target_length - current_length
        if sequence.ndim == 1:
            padding = np.tile(last_frame, padding_count)
        else:
            padding = np.tile(last_frame[np.newaxis, :], (padding_count, 1))
        return np.concatenate([sequence, padding], axis=0)

def extract_features(video_data, use_bbox=True, use_skeleton=True, use_angles=True, use_reduced_skeleton=True):
    """Extract features from video data (excluding centroids)"""
    frames = video_data.get("frames", [])
    features = []

    for frame in frames:
        frame_feats = []

        # BBox features
        if use_bbox:
            bbox = frame.get("bbox")
            if bbox:
                x_min, y_min, x_max, y_max = map(float, bbox)
                # area = max(1.0, (x_max - x_min) * (y_max - y_min))
                frame_feats.extend([x_min, y_min, x_max, y_max])
            else:
                frame_feats.extend([0.0] * 4)  # Placeholder for missing bbox

        # Skeleton features
        if use_skeleton:
            lm_key = "pose_landmarks_reduced" if use_reduced_skeleton else "pose_landmarks"
            lm = frame.get(lm_key)
            if lm:
                for point in lm:
                    frame_feats.extend([float(point[0]), float(point[1]), float(point[2]), float(point[3])])
            else:
                n_landmarks = 5 if use_reduced_skeleton else 33
                frame_feats.extend([0.0] * (n_landmarks * 4))
        
        # Angle features
        if use_angles:
            angles = frame.get("pose_angles")
            if angles:
                frame_feats.extend([float(v) for v in angles.values()])
            else:
                frame_feats.extend([0.0] * 3)
        
        features.append(frame_feats)
    
    return np.array(features, dtype=np.float32)

def extract_centroids(video_data):
    """Extract centroid coordinates from video data"""
    frames = video_data.get("frames", [])
    centroids = []

    for frame in frames:
        center = frame.get("center", [0.0, 0.0])
        centroids.append([float(center[0]), float(center[1])])

    return np.array(centroids, dtype=np.float32)

def build_trajectory_dataset(master_json, scenario_config, past_seconds=2.0, future_seconds=1.0):
    """Build dataset for trajectory prediction."""
    X_sequences, y_sequences, labels, frame_info = [], [], [], []
    max_past_frames, max_future_frames = 0, 0

    for video_name, video_data in master_json.items():
        fps = get_video_fps(video_data)

        # Convert time to frames
        past_frames = calculate_frames_from_seconds(fps, past_seconds)
        future_frames = max(1,calculate_frames_from_seconds(fps, future_seconds))

        # Max frames
        max_past_frames = max(max_past_frames, past_frames)
        max_future_frames = max(max_future_frames, future_frames)

        # Extract features and centroids
        features = extract_features(video_data, **scenario_config)
        centroids = extract_centroids(video_data)

        # Store frame information for this video
        video_frame_info = {
            "video_name": video_name,
            "fps": fps,
            "past_seconds": past_seconds,
            "future_seconds": future_seconds,
            "past_frames": past_frames,
            "future_frames": future_frames,
            "total_frames": len(features)
        }

        total_frames_needed = past_frames + future_frames
        n_frames = len(features)

        min_past = int(past_frames*0.75)
        min_future = int(future_frames*0.75)
        min_frames_threshold = min_past + min_future

        if n_frames < min_frames_threshold:
            continue

        # Take the tail of the video
        if n_frames >= total_frames_needed:
            # Video is too long, use tail
            start_idx = len(features) - total_frames_needed
            X_seq = features[start_idx:start_idx + past_frames]
            y_seq = centroids[start_idx + past_frames:start_idx + total_frames_needed]
        else:
            # Video is too short, use as is and pad
            past = max(1,min_past)
            future = max(1,min_future)
            excess = n_frames - min_past - min_future
            if excess > 0:
                future_boost = min(excess, future_frames - min_future)
                future += future_boost

                remaining_excess = excess - future_boost
                if remaining_excess > 0:
                    past_boost = min(remaining_excess, past_frames - min_past)
                    past += past_boost
            
            start_idx = max(0, n_frames - past - future)
            X_seq = features[start_idx:start_idx + past]
            y_seq = centroids[start_idx + past:start_idx + past + future]
        
        X_sequences.append(X_seq)
        y_sequences.append(y_seq)
        labels.append(video_data.get("label", "unknown"))

    # Pad all sequences to max length
    X_padded = [pad_sequences(seq, max_past_frames) for seq in X_sequences]
    y_padded = [pad_sequences(seq, max_future_frames) for seq in y_sequences]

    return X_padded, y_padded, labels, video_frame_info

def prepare_data(X_sequences, y_sequences, test_size=0.2):
    """Prepare and scale data for training."""
    # Convert to arrays
    X = np.array(X_sequences)
    y = np.array(y_sequences)

    # Split data
    X_train, X_test, y_train, y_test = train_test_split(
        X, y, test_size=test_size, random_state=42
    )

    # Scale features
    feature_scaler = StandardScaler()
    X_train_flat = X_train.reshape(-1, X_train.shape[-1])
    feature_scaler.fit(X_train_flat)

    X_train_scaled = feature_scaler.transform(X_train_flat).reshape(X_train.shape)
    X_test_scaled = feature_scaler.transform(X_test.reshape(-1, X_test.shape[-1])).reshape(X_test.shape)

    # Scale centroids
    centroid_scaler = StandardScaler()
    y_train_flat = y_train.reshape(-1, 2)
    centroid_scaler.fit(y_train_flat)

    y_train_scaled = centroid_scaler.transform(y_train_flat).reshape(y_train.shape)
    y_test_scaled = centroid_scaler.transform(y_test.reshape(-1, 2)).reshape(y_test.shape)

    return (X_train_scaled, X_test_scaled, y_train_scaled, y_test_scaled, feature_scaler, centroid_scaler)